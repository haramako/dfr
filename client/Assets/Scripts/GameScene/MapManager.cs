...
public class MapManager : MonoBehaviour
{
	...
	public override void OnFill (BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
	{
		...
		var center = View.MapCenter();
		var startX = ((int)center.x - 600) / 100;
		var startY = ((int)-center.y - 350) / 100;
		var endX = ((int)center.x + 600 + 99) / 100;
		var endY = ((int)-center.y + 350 + 99) / 100;

		float chip = 100 * tex.width / 2048;
		var size = 100;
		float d = 1f / tex.width;
		for (int y = startY; y < endY; y++) {
			for (int x = startX; x < endX; x++) {
				var id = View.MapView (mLayer, x, y);
				if (id < 0)	continue;
				var idx = id % 20;
				var idy = id / 20;
				Vector2 v1 = new Vector2 (x * size, -y * size);
				Vector2 v2 = new Vector2 (x * size + size, -y * size - size);
				Vector2 uv1 = new Vector2(idx*chip/tex.width+d, 1-((idy+1)*chip/tex.height)+d);
				Vector2 uv2 = new Vector2((idx+1)*chip/tex.width-d, 1-(idy*chip/tex.height)-d);

				verts.Add (new Vector3(v1.x, v2.y));
				verts.Add (new Vector3(v1.x, v1.y));
				verts.Add (new Vector3(v2.x, v1.y));
				verts.Add (new Vector3(v2.x, v2.y));

				uvs.Add (new Vector3(uv1.x, uv1.y));
				uvs.Add (new Vector3(uv1.x, uv2.y));
				uvs.Add (new Vector3(uv2.x, uv2.y));
				uvs.Add (new Vector3(uv2.x, uv1.y));
